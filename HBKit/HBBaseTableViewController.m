...
- (void)configTableView
{
    if ([self tableView]) {
        ...
        cell_struct *cell0struct = [self.dataDictionary cell_cellstructobjectForKey:KEY_INDEXPATH(0, 0)];
        if ([cell0struct.xibvalue isEqualToString:@"xib"]) {
            TABLEVIEW_REGISTERXIBCELL_CLASS(self.tableView, cell0struct.cellclass)
        }
        cell_struct *cellstruct1 = [self.dataDictionary cell_cellstructobjectForKey:KEY_INDEXPATH(1, 0)];
        if (cellstruct1) {
...
{
    self.tableView.frame = CGRectMake(left, self.tableView.frame.origin.y, [UIScreen mainScreen].bounds.size.width - left - right, self.tableView.frame.size.height);
...
{
    cell_struct *cell_struce = [self.dataDictionary cell_cellstructobjectForKey:KEY_INDEXPATH(section, 0)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (cell_struce.sectiontitle.length) {
        ...
        label.textColor = [cell_struct_common cell_colorWithStructKey:cell_struce.sectioncolor];
        label.textAlignment = NSTextAlignmentLeft;
        UIColor *bgcolor =  [cell_struct_common cell_colorWithStructKey:cell_struce.sectionbgcolor];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    if (cell_struce.sectionfooter.length) {
        ...
        label.font = [UIFont systemFontOfSize:sectionfont];
        label.textColor = [cell_struct_common cell_colorWithStructKey:cell_struce.sectionfootercolor];
        label.textAlignment = NSTextAlignmentLeft;
        UIColor *bgcolor =  [cell_struct_common cell_colorWithStructKey:cell_struce.sectionbgcolor];
        view.backgroundColor = bgcolor ? bgcolor : self.view.backgroundColor;
        [view addSubview:label];
...
{
    cell_struct *cellstruct = [self.dataDictionary cell_cellstructobjectForKey:KEY_INDEXPATH(indexPath.section, indexPath.row)];
