...
@interface HBDirWatchdog ()
@property (nonatomic, readonly) CFFileDescriptorRef kqRef;
- (void)kqueueFired;
@end


static void KHBQCallback(CFFileDescriptorRef kqRef, CFOptionFlags callBackTypes, void *info)
{
	// Pick up the object passed in the "info" member of the CFFileDescriptorContext passed to CFFileDescriptorCreate
    HBDirWatchdog *obj = (__bridge HBDirWatchdog *) info;
	
	if ([obj isKindOfClass:[HBDirWatchdog class]]		&&	// If we can call back to the proper sort of object ...
...
{
    return [[HBDirWatchdog alloc]initWithPath:[self documentsPath] update:update];
}
...
- (void)start
{
    ...
    _kqRef = CFFileDescriptorCreate(NULL,		// Use the default allocator
									...
									KHBQCallback,	// Fxn to call on activity
